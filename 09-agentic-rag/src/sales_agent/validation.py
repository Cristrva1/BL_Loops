"""Validador importable de las corridas JSONL del agente."""
...
class RunValidationError(ValueError):
    """La traza no respeta el contrato portable del laboratorio."""
...
def validate_run(path: Path) -> RunSummary:
    ...
    for expected, event in enumerate(events, 1):
        ...
        if event["run_id"] != run_id or event["lab_id"] != "09-agentic-rag":
...
def cli(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Valida una corrida de 09-agentic-rag.")
