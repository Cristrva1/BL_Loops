"""Importador minimo del JSONL exportado por el laboratorio."""
...
def validate_run(path: Path) -> RunSummary:
    ...
    for expected_sequence, event in enumerate(events, 1):
        if event["sequence"] != expected_sequence:
            raise RunValidationError("La secuencia de eventos no es monotonica y consecutiva.")
        if event["run_id"] != run_id:
            raise RunValidationError("Todos los eventos deben compartir el mismo run_id.")
        if event["lab_id"] != "06-naive-rag":
            raise RunValidationError("La corrida no pertenece a 06-naive-rag.")
...
def cli(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Importa una corrida de 06-naive-rag.")
    parser.add_argument("path", nargs="?", type=Path, help="JSONL; omitir usa el mas reciente.")
