"""Configuracion portable y fail-closed para el agente local."""
...
DEFAULT_INDEX_PATH = ".local/data/sales-library.sqlite3"
...
class Settings:
    ...
    def load(cls, root: Path | None = None) -> Settings:
        ...
        raw_embedding_model = env.get("SALES_AGENT_EMBEDDING_MODEL") or env.get(
            "BL_LOOPS_EMBEDDING_MODEL"
        )
        embedding_model = _model(
            raw_embedding_model or DEFAULT_EMBEDDING_MODEL,
            "SALES_AGENT_EMBEDDING_MODEL",
        )
        chat_model = _model(
            env.get(
                "SALES_AGENT_CHAT_MODEL",
                env.get("BL_LOOPS_MODEL_BASIC", DEFAULT_CHAT_MODEL),
            ),
            "SALES_AGENT_CHAT_MODEL",
        )
        try:
            timeout = float(env.get("SALES_AGENT_TIMEOUT_SECONDS", str(DEFAULT_TIMEOUT)))
        except ValueError as exc:
            raise ConfigurationError("SALES_AGENT_TIMEOUT_SECONDS debe ser numerico.") from exc
        if not 0 < timeout <= 1800:
            raise ConfigurationError("SALES_AGENT_TIMEOUT_SECONDS debe estar entre 0 y 1800.")

        raw_index = env.get("SALES_AGENT_INDEX_PATH")
        if raw_index is None:
            raw_index = str(
                Path(env.get("BL_LOOPS_DATA_DIR", ".local/data")) / "sales-library.sqlite3"
            )
        return cls(
            root=actual_root,
            ...
            embedding_dimensions=_integer(
                env, "SALES_AGENT_EMBEDDING_DIMENSIONS", DEFAULT_DIMENSIONS, 32, 4096
            ),
            embedding_batch_size=_integer(
                env, "SALES_AGENT_EMBEDDING_BATCH_SIZE", DEFAULT_BATCH_SIZE, 1, 64
            ),
            timeout_seconds=timeout,
            index_path=_lab_path(
                actual_root,
                raw_index.strip() or DEFAULT_INDEX_PATH,
                "SALES_AGENT_INDEX_PATH",
            ),
            runs_dir=_lab_path(
                actual_root,
                env.get(
                    "SALES_AGENT_RUNS_DIR",
                    env.get("BL_LOOPS_RUNS_DIR", DEFAULT_RUNS_DIR),
                ),
                "SALES_AGENT_RUNS_DIR",
            ),
            top_k=_integer(env, "SALES_AGENT_TOP_K", DEFAULT_TOP_K, 1, 12),
            chunk_chars=_integer(env, "SALES_AGENT_CHUNK_CHARS", DEFAULT_CHUNK_CHARS, 400, 4000),
            history_turns=_integer(env, "SALES_AGENT_HISTORY_TURNS", DEFAULT_HISTORY_TURNS, 0, 12),
        )
